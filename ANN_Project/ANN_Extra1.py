...
class NN:
    ...
    def show_result(self, ACC_t, show):
...
def get(input_hyper, f):
    s = input_hyper.split(" ")
    hyper = []
    for i in s:
        if(f):
            hyper.append(float(i))
        else:
            hyper.append(int(i))
    return hyper

epochs = input("Epochs: ")
l_rates = input("Learning rates: ")
b_sizes = input("Batch sizes: ")
print('$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$')
epochs = get(epochs, False)
l_rates = get(l_rates, True)
b_sizes = get(b_sizes, False)
...
for e in epochs:
    for b in b_sizes:
        for l_r in l_rates:
            case += 1
            print(f"Case {case}:")
            print(f"Epoch: {e}, Batch size: {b}, Learning rate: {l_r}")
            nn = NN(X_t, Y_t, 102, 4, e, l_r, b, "sigmoid")
            nn.train()
            nn.X = X_test
            nn.Y = Y_test
            ACC_t = nn.accuracy_cost()[0]
            nn.show_result(ACC_t, False)
            print('@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@')
            if(ACC_t > 0.98):
                E[e] += 1
                B[b] += 1
