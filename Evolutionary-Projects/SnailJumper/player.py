...
class Player(pygame.sprite.Sprite):
    def __init__(self, game_mode):
        ...
        if self.game_mode == "Neuroevolution":
            ...
            layer_sizes = [7, 10, 1]  # TODO (Design your architecture here by changing the values)
    ...
    def think(self, screen_width, screen_height, obstacles, player_x, player_y):
        ...
        x = self.create_input(screen_width, screen_height, obstacles, player_x, player_y, 7)
    ...
    def create_input(self, screen_width, screen_height, obstacles, player_x, player_y, n):
        x = [player_x / screen_width]
        i = 1
        j = 0
        while(i < n and j < len(obstacles)):
            if (obstacles[j]['y'] < player_y):
                x.append(obstacles[j]['x'] / screen_width)
                x.append(obstacles[j]['y'] / screen_height)
                i += 2
            j += 1
        while(i < n):
            x.append(200 / screen_width)
            x.append(700 / screen_height)
            i += 2
